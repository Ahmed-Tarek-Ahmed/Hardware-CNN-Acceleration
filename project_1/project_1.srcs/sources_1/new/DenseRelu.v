...
module DenseRelu #(parameter n_outputs = 128, n_inputs = 64, Input_size = 32,DP=4,SL=0,Relu=1)
    (
    ...
    for(i=1; i<=n_outputs; i = i+1)begin : Convolution
       Convolution #(.size(Input_size),.n_inputs(n_inputs),.DP(DP),.SL(SL),.Relu(Relu)) ui
       (
