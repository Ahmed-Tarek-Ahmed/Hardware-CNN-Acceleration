`timescale 1ns / 1ps
module top_test#(parameter samples=31, size=32,DP=5,paramD=57344,paramD1=8192,paramD2=320)
    ...
    wire [size*2240-1  : 0]WD;
    EMB_Layer #(.samples(samples),.size(size))EMB (
    .INBUS(In),
    .OUTBUS(data)
    );

    RAM #(size)ram(WT);Ram2 #(size)ram2(WT2);RAMD2#(size)ramD(WD);
    ...
    DenseRelu#(.n_outputs(D2F), .n_inputs(448), .Input_size(size),.DP(DP),.SL(2),.Relu(0))D2
                    (
                        .INBUS(InDense),
                        .weights(WD),
                        .OUTBUS(OutDense2)
